...
class PlayState extends MusicBeatState
{
	...
	function popupWindow(customWidth:Int, customHeight:Int, ?customX:Int, ?customName:String) {
        ...
        windowDad.x = Std.int(display.height / 2);
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		if(Math.isNaN(intValue1)) intValue1 = null;
		if(Math.isNaN(intValue2)) intValue2 = null;

		switch(eventName) {
			case 'Get dad out of window':
				popupWindow(intValue1, intValue2, 100, 'Me!');
	...
	public function endSong()
	{
		...
		if(ret != LuaUtils.Function_Stop && !transitioning)
		{
			...
			Highscore.saveScore(SONG.song, songScore, songMisses, ratingFC, storyDifficulty, percent);
			#end
			playbackRate = 1;
			...
			if (isStoryMode)
			{
				...
				{
					Mods.loadTopMod();
					FlxG.sound.playMusic(Paths.music('menu post cancion'), 1, true);
					#if DISCORD_ALLOWED DiscordClient.resetClientID(); #end

					MusicBeatState.switchState(new MainMenuState());

					if (SONG.song == "Piero")	ClientPrefs.data.alrBeatPiero = true;
					if (SONG.song == "Piero 2")	ClientPrefs.data.alrBeatPiero2 = true;
					if (SONG.song == "Piero d")	ClientPrefs.data.alrBeatPieroD = true;

					trace(ClientPrefs.data.alrBeatPiero);
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.x += ClientPrefs.data.comboOffset[0] + 950; 
		rating.y -= ClientPrefs.data.comboOffset[1] - 300;
		...
		comboSpr.x += ClientPrefs.data.comboOffset[0] + 1000;
		comboSpr.y -= ClientPrefs.data.comboOffset[1] - 300;
		...
		for (i in seperatedScore)
		{
			...
			numScore.x = placement + (43 * daLoop) - 90 + ClientPrefs.data.comboOffset[2] + 1050;
			numScore.y += 80 - ClientPrefs.data.comboOffset[3] + 300;
